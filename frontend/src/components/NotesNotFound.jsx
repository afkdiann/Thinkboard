import { NotebookIcon } from "lucide-react";
import { Link } from "react-router";

const NotesNotFound = () => {
  return (
    <div className="flex flex-col items-center justify-center py-16 space-y-6 max-w-md mx-auto text-center">
      <div className="rounded-full p-8">
        <NotebookIcon className="size-10 text-primary text-green-500" />
      </div>
      <h3 className="text-2xl font-bold text-white">No notes yet</h3>
      <p className="text-gray-500">
        Ready to organize your thoughts? Create your first note to get started
        on your journey.
      </p>
      <Link to={"/create"}>
        <button className="font-medium border-2 rounded-2xl bg-green-500 p-2">Create your first note</button>
      </Link>
    </div>
  );
};

export default NotesNotFound;
